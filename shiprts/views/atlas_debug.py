...
class AtlasView(View):
    ...
    def __init__(self):
        ...
        self.cam = arcade.Camera2D(position=(self.sheet_texture.width/2.0, self.sheet_texture.height/2.0))
        self.current_texture_idx = 0

    def on_draw(self) -> None:
        self.clear()
        with self.cam.activate():
            arcade.draw_texture_rect(self.sheet_texture, arcade.LBWH(0.0, 0.0, self.sheet_texture.width, self.sheet_texture.height), pixelated=False)
    ...
    def on_key_press(self, symbol: int, modifiers: int) -> bool | None:
        if symbol == arcade.key.R:
            ...
            self.current_texture_idx = (self.current_texture_idx + 1) % len(self.toml_data['texture'])
